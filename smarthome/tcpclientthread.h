...
public:
  ...
  /*函数功能：传感器数据主动上传
    ...
    函数返回：无*/
    void upload(QString type,QString cmdCode,QString value);
signals:
  /*函数功能：接收服务端数据处理后，若数据处理后为设备控制，则向主线程发出信号
    函数参数：type:传感器类型，详情见传感器字段定义；command:控制命令，详情见协议字段表
    函数返回：无*/
    void evt_socket_receive(QString type,unsigned int command,unsigned int cmdCode,unsigned int channel);
public slots:
